(() => {
  ...
  const onMouseUp = () => {
    ...
    const column_id = placeholder.closest(".column").getAttribute('data-column-id')
    const card_id = placeholder.previousElementSibling.children[0].getAttribute('data-card-id')

    fetch(`http://localhost:8000/trello/card-place/`, {
        method: 'POST',
        body: {
            column_id,
            card_id
